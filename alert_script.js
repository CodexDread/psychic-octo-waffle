...
class MosaicFallback {
  ...
  appear(ms){ anime.set(this.cells,{opacity:1}); this.host.style.display='block';
  ...
  disappear(ms){ anime.set(this.cells,{opacity:0}); this.host.style.display='block';
...
class AlertCard {
  ...
  stopScramble(){
    ...
    return anime({ targets:{p:0}, p:1, duration:260, easing:'easeOutQuad',
  ...
  async run(){
    ...
    if (this._isAd){
      if (CONFIG.BAR_POS==='Top' || CONFIG.BAR_POS==='Bottom'){
        ...
        await (anime({ targets:this.bar, width:['100%','0%'], duration:this._durationMs, easing:'linear',
